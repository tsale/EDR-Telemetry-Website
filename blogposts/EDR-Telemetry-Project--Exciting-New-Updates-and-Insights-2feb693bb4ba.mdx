...
Telemetry categorized as “Via EventLogs” refers to data collected from platform-native event logs rather than independently collected by the EDR’s own sensor instrumentation. On Windows, this typically means Windows Event Logs rather than **Event Tracing for Windows (ETW)**. On Linux and macOS, the same status can apply when telemetry is exposed by ingesting OS-managed logs such as syslog/auth logs, journald, or Unified Logging, depending on the platform and configuration. This distinction helps clarify reliance on system-level logging sources and their configuration.
