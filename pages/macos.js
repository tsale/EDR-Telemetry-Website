...
import { MacosLogo } from '../components/PlatformOsIcons'
...
export default function MacOS() {
  ...
  const customStyles = `
    ...
    /* Sysmon styling (Sysmon for macOS) */
    .sysmon-header {
  ...
  // Sort telemetry data (Sysmon first, then others)
  ...
  // Load telemetry data from macOS endpoint
  ...
  const renderTelemetryTable = useCallback(() => {
              ...
              <tr>
                ...
                {displayedEdrs.map(edr => {
                  ...
                  return (
                    <th
                      key={edr}
                      className={`edr-column ${isSysmon ? 'sysmon-header' : ''}`}
                    >
                      ...
                      {displayedEdrs.map(edr => {
                        ...
                        return (
                          <td
                            key={statusKey}
                            data-status={status}
                            className={`${isSysmon ? 'sysmon-column' : ''}`}
                          >
    ...
    // For normal display mode, rearrange to put Sysmon first (after first two columns)
    const sysmonEdr = displayedEdrs.find(edr => edr.toLowerCase().includes('sysmon'));

    // Remove Sysmon from the array if it exists
    if (sysmonEdr) {
      displayedEdrs = displayedEdrs.filter(edr => edr !== sysmonEdr);
    }

    // Add Sysmon at the beginning
    const orderedEdrs = [];
  ...
  return (
    <TemplatePage title="macOS EDR Telemetry Analysis"
      description="Vendor-neutral analysis of macOS EDR telemetry coverage, categories, and depth.">
      <Head>
      ...
      <section className="relative bg-slate-900 text-white pt-12 pb-16 sm:pt-20 sm:pb-24 overflow-hidden">
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute -top-[30%] -left-[10%] w-[70%] h-[70%] rounded-full bg-purple-900/20 blur-[100px]"></div>
          <div className="absolute -bottom-[30%] -right-[10%] w-[70%] h-[70%] rounded-full bg-violet-900/20 blur-[100px]"></div>
        </div>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 z-10 text-center flex flex-col items-center justify-center min-h-0 sm:min-h-[420px]">
          <div className="inline-flex items-center justify-center p-5 bg-purple-600/20 rounded-xl mb-8 backdrop-blur-sm ring-1 ring-purple-500/50">
            <MacosLogo className="w-12 h-12 text-white" />
          </div>

          <h1 className="text-3xl sm:text-4xl md:text-6xl font-extrabold tracking-tight mb-6 !text-white leading-tight">
            macOS EDR <span className="text-purple-400">Telemetry</span>
          </h1>

          <p className="mt-4 max-w-2xl mx-auto text-base sm:text-xl text-slate-300 leading-relaxed !text-center">
            Explore detailed telemetry capabilities and comparisons for macOS-based EDR solutions.
      ...
      <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 -mt-8 sm:-mt-12 relative z-20">
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
              ...
              <div className="w-full lg:w-auto flex flex-col sm:flex-row gap-4">
                <div className="relative group w-full min-w-0 sm:min-w-[280px]">
                  <input
                    type="text"
                    className="block w-full !py-2.5 !pl-3 !pr-10 border border-slate-200 rounded-xl leading-5 bg-white shadow-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 sm:text-sm transition-all"
                    ...
                    <>
                      ...
                      <div className="absolute mt-1 w-full bg-white shadow-xl max-h-96 rounded-lg py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm z-[110]">
                        {filteredEdrOptions.map(edr => (
                          <div
                            key={edr}
                            className="cursor-pointer select-none relative py-3 pl-3 pr-9 hover:bg-purple-50 text-slate-700 flex items-center"
                            onClick={() => addToComparison(edr)}
                          >
                            <span className={`block truncate font-medium ${selectedEDRs.includes(edr) ? 'text-purple-600' : ''}`}>
                              {edr}
                            </span>
                ...
                <div className="flex items-center space-x-2 bg-white px-3 py-2 border border-slate-300 rounded-lg shadow-sm">
                  <label htmlFor="hoverToggle" className="flex items-center cursor-pointer select-none">
                    <div className="relative">
                      ...
                      />
                      <div className={`block w-10 h-6 rounded-full transition-colors ${hoverEnabled ? 'bg-purple-500' : 'bg-slate-300'}`}></div>
            ...
            {selectedEDRs.length > 0 && (
              <div className="mt-4 flex flex-wrap gap-2">
                {selectedEDRs.map(edr => (
                  <span key={edr} className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-purple-100 text-purple-800 border border-purple-200">
                    {edr}
                    <button
                      type="button"
                      className="flex-shrink-0 ml-1.5 h-4 w-4 rounded-full inline-flex items-center justify-center text-purple-600 hover:bg-purple-200 hover:text-purple-500 focus:outline-none"
          ...
          <div className="p-0">
            ...
            <div className="bg-purple-50/50 border-b border-purple-100 px-6 py-3 flex items-start sm:items-center gap-3">
              <HelpCircle className="w-5 h-5 text-purple-500 flex-shrink-0 mt-0.5 sm:mt-0" />
