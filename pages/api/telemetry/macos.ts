...
import type { Database, MacosTelemetryWithScores } from '../../../utils/supabase/types'
...
export default async function handler(
  ...
  try {
    ...
    switch (req.method) {
      case 'GET':
        // Fetch macOS telemetry data with scores
        ...
        const transformedData = (telemetryData as MacosTelemetryWithScores[]).map(item => {
          const result: MacosTelemetryResult = {
          ...
          item.macos_table_results.forEach(score => {
