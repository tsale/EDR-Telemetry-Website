...
export default function About() {
  ...
  return (
    <TemplatePage title={title} description={description} ogTitle="About the EDR Telemetry Project" ogDescription="We document which endpoint events EDR products expose on Windows, Linux, and macOS under controlled, versioned tests." canonicalPath="/about">
      <Head>
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
          '@context': 'https://schema.org', '@type': 'AboutPage', name: 'About the EDR Telemetry Project',
          url: `${SITE_URL}/about`, description: 'Public research comparing EDR endpoint telemetry visibility under controlled, versioned tests.',
          ...
          <div className="grid gap-8 md:grid-cols-[minmax(0,1.4fr)_minmax(15rem,0.8fr)] md:items-center md:gap-10 lg:gap-14">
            <div className="max-w-2xl">
              ...
              <p className="max-w-xl text-lg leading-relaxed !text-slate-300 sm:text-xl">
                We document which endpoint events EDR products expose under controlled, versioned
                tests.
              </p>
            </div>
      ...
      <section className="bg-slate-50 py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
          <div>
            <div className="mx-auto mb-10 max-w-3xl text-center">
              ...
              <p className="text-lg leading-relaxed text-slate-600">
                ...
                maintains this as open research. We document and compare the endpoint telemetry EDR
                platforms expose under controlled, versioned tests.
              </p>
            </div>

            <div className="grid gap-6 md:grid-cols-3">
              <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:-translate-y-0.5 hover:border-blue-200 hover:shadow-md">
                <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-xl bg-blue-600 text-white shadow-md shadow-blue-600/20">
                  <Search className="h-5 w-5" aria-hidden="true" />
                </div>
                <h3 className="mb-2 text-lg font-bold text-slate-900">Telemetry visibility</h3>
                <p className="mb-0 text-sm leading-relaxed text-slate-600">
                  We test which endpoint events EDR platforms collect and expose for investigation,
                  under documented conditions.
                </p>
              </article>

              <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:-translate-y-0.5 hover:border-emerald-200 hover:shadow-md">
                <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-xl bg-emerald-600 text-white shadow-md shadow-emerald-600/20">
                  <Users className="h-5 w-5" aria-hidden="true" />
                </div>
                <h3 className="mb-2 text-lg font-bold text-slate-900">Built for practitioners</h3>
                <p className="mb-0 text-sm leading-relaxed text-slate-600">
                  Use the results to compare visibility gaps, validate a deployment, or gather
                  evidence for a platform choice. Scores cover exposed telemetry. They leave
                  prevention quality, detection efficacy, and managed-service quality out of scope.
                </p>
              </article>

              <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:-translate-y-0.5 hover:border-indigo-200 hover:shadow-md">
                <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-xl bg-indigo-600 text-white shadow-md shadow-indigo-600/20">
                  <Eye className="h-5 w-5" aria-hidden="true" />
                </div>
                <h3 className="mb-2 text-lg font-bold text-slate-900">Results you can recheck</h3>
                <p className="mb-0 text-sm leading-relaxed text-slate-600">
                  We publish what we tested, what evidence we saw, how we classified it, and which
                  limits still apply.
                </p>
              </article>
            </div>
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            <article className="bg-white rounded-2xl shadow-lg p-8 border border-slate-200"><h2 className="text-2xl font-bold text-slate-900 mb-4">In scope</h2><p className="text-slate-600 mb-6">We measure telemetry visibility exposed to product consumers: raw or near-raw endpoint event data you can search or use for investigation, hunting, or detection engineering.</p><Checklist items={measures} /></article>
            <article className="bg-white rounded-2xl shadow-lg p-8 border border-slate-200"><h2 className="text-2xl font-bold text-slate-900 mb-6">Out of scope</h2><Checklist items={doesNotMeasure} /></article>
          </div>
          ...
          <div>
            ...
            <p className="text-lg text-slate-600 mb-8 max-w-4xl">We design tests so each conclusion traces to a controlled action, the environment, the product evidence, and the methodology version used to classify the result.</p>
          ...
          <div className="bg-amber-50 rounded-2xl p-8 border border-amber-200">
            <h2 className="text-3xl font-bold text-slate-900 mb-4">Limits of use</h2>
            <p className="text-lg text-slate-700 mb-6">Telemetry is one evaluation input. Pair it with detection quality, response capabilities, operational fit, integrations, staffing, infrastructure, licensing, compliance needs, vendor support, and other local factors.</p>
            <Checklist items={['Does not rank EDRs overall', 'Does not test prevention', 'Does not test detection efficacy', 'Does not evaluate MDR', 'Does not replace a client-specific proof of value', 'Does not guarantee identical telemetry across every deployment or product version']} />
          </div>
          ...
          <div>
            ...
            <div className="mt-6 flex flex-wrap gap-4"><Link href="/contribute" className="text-blue-600 hover:underline font-semibold">Contribute to the Project</Link><Link href="/support" className="text-blue-600 hover:underline font-semibold">Support the Project</Link></div>
          </div>
